...
\maketitle
\begin{abstract}
    作为人机交互的未来，人们对于虚拟现实（VR）的画质要求越来越高，作为一种光线追踪算法，光子映射算法能满足这个需求。但是，由于VR交互需要很强的实时性，一
    般的未经优化的光子映射算法无法满足要求。这里我们提出了基于GPU共享内存和VR渲染双眼交叉特性的改善性能的方法，使得在保证渲染效果下得到最佳的实时交互体验。
\end{abstract}
...
\section{不足和改进}
    ...
    而且由于人眼的特性，渲染的精细度可以依据像素位置来调整，使得中心区域的采样素密度达到最高（而且这块区域一般是被两眼共享的，也就是说对于同一个采样点基
    本上只需要收集一次光子），而周围区域采样密度逐渐递减。这样可以在不影响观感的前提下进一步提升渲染。
\bibliography{ref}
